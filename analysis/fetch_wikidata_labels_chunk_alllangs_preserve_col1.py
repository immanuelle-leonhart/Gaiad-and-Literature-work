...
UA = "WikidataSPARQLLabelDump/2.1 (contact: your-email@example.com)"
...
def read_rows_from_csv(path):
    """
    Return a list of (col1, qid) rows in original order, filtering to rows with a valid QID in col 2.
    """
    rows = []
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if re.match(r"^Q\d+$", qid):
                rows.append((col1, qid))
    return rows

def dedupe_preserve_order_qids(rows):
    """
    Given list of (col1, qid), return a QID list in first-seen order.
    """
    seen = set()
    out = []
    for _, q in rows:
...
def main():
    ap = argparse.ArgumentParser(description="Fetch all labels & descriptions (all languages) via SPARQL per chunk; preserve column 1.")
    ap.add_argument("input_csv", help="Input CSV with (col1, QID) in first two columns.")
    ...
    rows = read_rows_from_csv(args.input_csv)  # [(col1, qid), ...]
    if not rows:
        print("No rows with a valid QID in column 2 were found.", file=sys.stderr)
        sys.exit(1)

    qids = dedupe_preserve_order_qids(rows)
    print(f"Total input rows: {len(rows)}   Unique QIDs: {len(qids)}")
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        try:
            for qid, lang, label in fetch_all_labels_for_chunk(session, chunk):
        ...
        try:
            for qid, lang, desc in fetch_all_descs_for_chunk(session, chunk):
                descs[qid][lang] = desc
                langset.add(lang)
        except Exception as e:
    ...
    # Write CSV: col1_id, wikidata_id, then 2 columns per language
    header = ["col1_id", "wikidata_id"]
