#!/usr/bin/env python3
"""
GEDCOM 1200 CE Trimmer
Removes individuals born after 1200 CE who lack GENI identifiers
Preserves all GENI-linked people and pre-1200 CE individuals
"""
...
def should_keep_individual(birth_year, has_geni, has_wikidata):
    """Determine if individual should be kept based on trimming rules"""
    # Keep if born before 1200 CE (essential antiquity)
    if birth_year is None or birth_year < 1200:
        return True
    ...
    # Remove if born after 1200 CE and no GENI identifier
    return False

def trim_gedcom_1200(input_file, output_file):
    """Trim GEDCOM file using 1200 CE cutoff with GENI preservation"""
    print(f"Trimming GEDCOM: {input_file}")
    print(f"Output file: {output_file}")
    print("Rules: Keep pre-1200 CE + all GENI identifiers")
        ...
        with open(input_file, 'r', encoding='latin1') as infile, \
            ...
            for line in infile:
                ...
                # Family record
                    ...
                    in_family = True
                    skip_current_record = False
                    current_family_valid = False  # Will validate as we read
                    # Don't write family line yet, buffer it
                    buffered_family_lines = [line]
                
                # Other level 0 records (SUBM, SOUR, etc.)
                ...
                # Content within records
                    ...
                    if in_individual:
                        ...
                        # End of family record - decide whether to write it
                        if stripped_line.startswith('0 ') or not stripped_line:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python gedcom_1200_trimmer.py <input_gedcom> <output_gedcom>")
        print("Example: python gedcom_1200_trimmer.py merged_attempt_cleaned.ged gaiad_trimmed_1200.ged")
        sys.exit(1)
    ...
    success = trim_gedcom_1200(input_file, output_file)
    if success:
