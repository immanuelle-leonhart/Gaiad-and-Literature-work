#!/usr/bin/env python3
"""
Create the final 34 missing individuals
"""
...
def create_session():
    ...
    session.headers.update({
        'User-Agent': 'Final Missing Individuals Creator/1.0 (https://github.com/Immanuelle/Gaiad-Genealogy; immanuelle@example.com)'
    })
...
def create_wikibase_item(session, csrf_token, individual_data):
    ...
    if name_parts:
        # Use "Lord " prefix to force uniqueness and avoid wikibase duplicate detection
        labels['en'] = {'language': 'en', 'value': 'Lord ' + ' '.join(name_parts)}
    else:
        labels['en'] = {'language': 'en', 'value': f'Lord Person {individual_data["gedcom_id"]}'}
    
    descriptions = {'en': {'language': 'en', 'value': 'Gaiad character with Lord title'}}
...
def main():
    print("Creating final missing individuals...")
