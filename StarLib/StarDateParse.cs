...
namespace StarLib
{
    ...
    class StarDateParse
    {
        ...
        internal const Int32 MaxStarDateNumberDigits = 8;

        internal delegate bool MatchNumberDelegate(ref __DTString str, int digitLen, out int result);

        internal static MatchNumberDelegate m_hebrewNumberParser = new MatchNumberDelegate(StarDateParse.MatchHebrewDigits);

#if !FEATURE_CORECLR
        [SecuritySafeCritical]
        internal static bool GetAmPmParseFlag()
        {
            return StarDate.EnableAmPmParseAdjustment();
        }
        ...
        internal static StarDate ParseExact(String s, String format, StarCulture dtfi, StarDateStyles style)
        {
            ...
            {
                throw GetStarDateParseException(ref result);
            }
        }
                                                ...
                                                StarCulture dtfi, StarDateStyles style, ref StarDateResult result)
        {
            ...
            for (int i = 0; i < formats.Length; i++)
            {
                ...
                StarDateResult innerResult = new StarDateResult();       // The buffer to store the parsing result.
                innerResult.Init();
        ...
        ////////////////////////////////////////////////////////////////////////////
        //
        // StarDate parsing state enumeration (DS.*)
        //
        ////////////////////////////////////////////////////////////////////////////
...
new DS[] { DS.ERROR, DS.TX_NNN,  DS.TX_NNN,  DS.TX_NNN,  DS.ERROR,   DS.ERROR,   DS.ERROR,   DS.ERROR,   DS.ERROR,   DS.ERROR,   DS.T_S,         DS.T_NNt,     DS.ERROR,   DS.ERROR,   DS.ERROR,   DS.T_NNt,   DS.T_NNt,   DS.TX_NNN},
        ...
        **          "+8", "+08", "+0800", "+0800"
        **        This method is used by StarDate.Parse().
        ...
        **      FormatException if invalid timezone format is found.
        ...
        private static bool HandleTimeZone(ref __DTString str, ref StarDateResult result)
        {
        ...
        private static Boolean Lex(DS dps, ref __DTString str, ref StarDateToken dtok, ref StarDateRawInfo raw, ref StarDateResult result, ref StarCulture dtfi, StarDateStyles styles)
        {
            ...
            if (_tracingEnabled) {
                BCLDebug.Trace("StarDate", "[StarDate] Lex({0})\tpos:{1}({2}), {3}, DS.{4}", Hex(str.Value),
            ...
            switch (tokenType)
            {
                ...
                case TokenType.YearNumberToken:
                    ...
                    switch (sep = str.GetSeparatorToken(dtfi, out indexBeforeSeparator, out charBeforeSeparator))
                    {
                        ...
                        case TokenType.SEP_YearSuff:
                            ...
                            {
                                result.SetFailure(ParseFailureKind.Format, "Format_BadStarDate", e);
                ...
                case TokenType.UnknownToken:
                    ...
                    {
                        result.SetFailure(ParseFailureKind.FormatWithParameter, "Format_UnknowStarDateWord", str.Index);
                    ...
                    // Visual Basic implements string to date conversions on top of StarDate.Parse:
                    //   CDate("#10/10/95#")
                    //
        ...
        //
        private static bool TryAdjustYear(ref StarDateResult result, int year, out int adjustedYear)
        {
            if (year < 100)
            {
                try
        ...
        private static bool SetDateYMD(ref StarDateResult result, int year, int month, int day)
        {
        ...
        private static bool SetDateMDY(ref StarDateResult result, int month, int day, int year)
        {
            return (SetDateYMD(ref result, year, month, day));
        }

        private static bool SetDateDMY(ref StarDateResult result, int day, int month, int year)
        {
            return (SetDateYMD(ref result, year, month, day));
        }

        private static bool SetDateYDM(ref StarDateResult result, int year, int day, int month)
        {
            return (SetDateYMD(ref result, year, month, day));
        }
        ...
        //
        private static Boolean GetDateOfDSN(ref StarDateResult result, ref StarDateRawInfo raw)
        {
        ...
        private static Boolean GetDateOfNDS(ref StarDateResult result, ref StarDateRawInfo raw)
        {
            if (result.Month == -1)
            {
        ...
        //
        private static bool ProcessStarDateSuffix(ref StarDateResult result, ref StarDateRawInfo raw, ref StarDateToken dtok)
        {
            switch (dtok.suffix)
            {
                case TokenType.SEP_YearSuff:
        ...
        //
        // Actions:
        // This is used by StarDate.Parse().
        ...
        internal static bool TryParse(String s, StarCulture dtfi, StarDateStyles styles, out StarDate result, out TimeSpan offset)
        {
            result = StarDate.MinValue;
            offset = TimeSpan.Zero;
            StarDateResult parseResult = new StarDateResult();       // The buffer to store the parsing result.
            parseResult.Init();
        ...
        internal static bool TryParse(String s, StarCulture dtfi, StarDateStyles styles, ref StarDateResult result)
        {
            ...
            StarDateToken dtok = new StarDateToken();      // The buffer to store the parsing token.
            dtok.suffix = TokenType.SEP_Unk;
            StarDateRawInfo raw = new StarDateRawInfo();    // The buffer to store temporary parsing information.
            unsafe
            {
            ...
            do
            {
                ...
                if (dtok.dtt != DTT.Unk)
                {
                    ...
                    {
                        if (!ProcessStarDateSuffix(ref result, ref raw, ref dtok))
                        {
                            result.SetFailure(ParseFailureKind.Format, "Format_BadStarDate", null);
                    ...
                    {
                        if ((dtfi.FormatFlags & StarDateFormatFlags.UseHebrewRule) != 0)
                        {
            ...
            {
                result.SetFailure(ParseFailureKind.FormatBadStarDateCalendar, "Format_BadStarDateCalendar", null);
                TPTraceExit("0100 (result.calendar.TryToStarDate)", dps);
                return false;
            }
            if (raw.fraction > 0)
            {
        ...
        private static Boolean DetermineTimeZoneAdjustments(ref StarDateResult result, StarDateStyles styles, Boolean bTimeOnly)
        {
            ...
            if ((result.flags & ParseFlags.TimeZoneUsed) == 0)
            {
                ...
                // to exceed StarDate.MaxValue
                if ((styles & StarDateStyles.AssumeLocal) != 0)
                {
                    ...
                    {
                        result.parsedDate = StarDate.SpecifyKind(result.parsedDate, DateTimeKind.Local);
                        return true;
                    }
                }
                else if ((styles & StarDateStyles.AssumeUniversal) != 0)
                {
                    if ((styles & StarDateStyles.AdjustToUniversal) != 0)
                    {
                        result.parsedDate = StarDate.SpecifyKind(result.parsedDate, DateTimeKind.Utc);
                        return true;
                    }
                    else
                    {
            ...
            if (((styles & StarDateStyles.RoundtripKind) != 0) && ((result.flags & ParseFlags.TimeZoneUtc) != 0))
            {
                result.parsedDate = StarDate.SpecifyKind(result.parsedDate, DateTimeKind.Utc);
                return true;
            }
        ...
        private static Boolean StarDateTimeZonePostProcessing(ref StarDateResult result, StarDateStyles styles)
        {
            ...
            if ((result.flags & ParseFlags.TimeZoneUsed) == 0)
            {
                if ((styles & StarDateStyles.AssumeUniversal) != 0)
                {
            ...
            if ((styles & StarDateStyles.AdjustToUniversal) != 0)
            {
                if (((result.flags & ParseFlags.TimeZoneUsed) == 0) && ((styles & StarDateStyles.AssumeUniversal) == 0))
                {
        ...
        private static Boolean AdjustTimeZoneToUniversal(ref StarDateResult result)
        {
            ...
            if (resultTicks < 0)
            {
                resultTicks += StarDate.TicksPerDay;
            }
            ...
            result.parsedDate = new StarDate(resultTicks, DateTimeKind.Utc);
            return true;
        }
        ...
        private static Boolean AdjustTimeZoneToLocal(ref StarDateResult result, bool bTimeOnly)
        {
            ...
            if (resultTicks < StarDate.TicksPerDay)
            {
                //
                // This is time of day.
                //
                ...
                resultTicks += (long)tz.GetUtcOffset(bTimeOnly ? StarDate.Now : result.parsedDate, TimeZoneInfoOptions.NoThrowOnInvalidTime).Ticks;
            ...
            else
            {
                ...
                else
                {
                    ...
                    resultTicks += StarZone.GetUtcOffsetFromUtc(utcDt, (StarZone)TimeZoneInfo.Local, out isDaylightSavings, out isAmbiguousLocalDst).Ticks;
                }
            }
            if (resultTicks < StarDate.MinTicks || resultTicks > StarDate.MaxTicks)
            {
                result.parsedDate = StarDate.MinValue;
                result.SetFailure(ParseFailureKind.Format, "Format_DateOutOfRange", null);
                return false;
            }
            result.parsedDate = new StarDate(resultTicks, DateTimeKind.Local, isAmbiguousLocalDst);
            return true;
        }
        ...
        //
        //
        private static bool ParseISO8601(ref StarDateRawInfo raw, ref __DTString str, StarDateStyles styles, ref StarDateResult result)
        {
            ...
            if (str.GetNext())
            {
                ...
                if (str.GetNext())
                {
                    // If this is true, there were non-white space characters remaining in the StarDate
                    result.SetFailure(ParseFailureKind.Format, "Format_BadStarDate", null);
                    return false;
                }
            }
            ...
            StarDate time;
            StarCulture calendar = StarCulture.InvariantCulture;
        ...
        //      This is used by StarDate.ParseExact().
        //
        ////////////////////////////////////////////////////////////////////////
        ...
        **              symbol is not found.
        ...
        /*=================================ParseTimeZoneOffset==================================
        **Action: Parse the string formatted using "z", "zz", "zzz" in StarDate.Format().
        ...
        **Exceptions: FormatException if a month name can not be found.
        ...
        private static bool MatchMonthName(ref __DTString str, StarCulture dtfi, ref int result)
        {
            ...
            if (str.GetNext())
            {
                ...
                if ((dtfi.FormatFlags & StarDateFormatFlags.UseGenitiveMonth) != 0)
                {
        ...
        **Exceptions:
        ...
        private static bool CheckNewValue(ref int currentValue, int newValue, char patternChar, ref StarDateResult result)
        {
            if (currentValue == -1)
            {
            ...
            else
            {
                ...
                {
                    result.SetFailure(ParseFailureKind.FormatWithParameter, "Format_RepeatStarDatePattern", patternChar);
                    return (false);
                }
            }
            return (true);
        ...
        private static StarDate GetStarDateNow(ref StarDateResult result, ref StarDateStyles styles)
        {
            if ((result.flags & ParseFlags.CaptureOffset) != 0)
            {
                if ((result.flags & ParseFlags.TimeZoneUsed) != 0)
                {
                    ...
                    return new StarDate(StarDate.UtcNow.Ticks + result.timeZoneOffset.Ticks, DateTimeKind.Unspecified);
                }
                else if ((styles & StarDateStyles.AssumeUniversal) != 0)
                {
                    // assume the offset is Utc
                    return StarDate.UtcNow;
                }
            }
        ...
        private static bool CheckDefaultStarDate(ref StarDateResult result, ref StarCulture cal, StarDateStyles styles)
        {
            ...
            if ((result.Year == -1) || (result.Month == -1) || (result.Day == -1))
            {
                ...
                */
                ...
                StarDate now = GetStarDateNow(ref result, ref styles);
                if (result.Month == -1 && result.Day == -1)
                {
                    if (result.Year == -1)
                    {
                        if ((styles & StarDateStyles.NoCurrentDateDefault) != 0)
                        {
                            ...
                            cal = StarCulture.InvariantCulture;
        ...
        private static String ExpandPredefinedFormat(String format, ref StarCulture dtfi, ref ParsingInfo parseInfo, ref StarDateResult result)
        {
            ...
            switch (format[0])
            {
                ...
                case 'O':       // Round Trip Format
                    parseInfo.calendar = StarCulture.InvariantCulture;
                ...
                case 'U':       // Universal time format with culture-dependent format.
                    ...
                    result.timeZoneOffset = new TimeSpan(0);
                    result.flags |= ParseFlags.TimeZoneUtc;
                    if (true) //(dtfi.GetType() != typeof(GregorianCalendar))
                    {
                        dtfi = (StarCulture)dtfi.Clone();
                        dtfi = StarCulture.InvariantCulture;
                    }
                    break;
            }
            ...
            ref StarDateResult result)
        {
            ...
            switch (ch)
            {
                ...
                case 'F':
                    tokenLen = format.GetRepeatCount();
                    if (tokenLen <= StarDateFormat.MaxSecondsFractionDigits)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                result.SetFailure(ParseFailureKind.FormatWithParameter, "Format_RepeatStarDatePattern", ch);
                                return (false);
                            }
                        }
                    }
                ...
                case 'z':
                    ...
                    tokenLen = format.GetRepeatCount();
                    {
                        ...
                        {
                            result.SetFailure(ParseFailureKind.FormatWithParameter, "Format_RepeatStarDatePattern", 'z');
                            return (false);
                        }
                ...
                case 'K':
                    ...
                    if (str.Match('Z'))
                    {
                        if ((result.flags & ParseFlags.TimeZoneUsed) != 0 && result.timeZoneOffset != TimeSpan.Zero)
                        {
                            result.SetFailure(ParseFailureKind.FormatWithParameter, "Format_RepeatStarDatePattern", 'K');
                            return (false);
                        }
                ...
                default:
                    ...
                    else
                    {
                        if (format.MatchSpecifiedWord(GMTName))
                        {
                            ...
                            // Found GMT string in format.  This means the StarDate string
                            // is in GMT timezone.
                            result.flags |= ParseFlags.TimeZoneUsed;
                            result.timeZoneOffset = TimeSpan.Zero;
        ...
        /*=================================DoStrictParse==================================
        **Action: Do StarDate parsing using the format in formatParam.
        **Returns: The parsed StarDate.
        **Arguments:
        **Exceptions:
        **
        **Notes:
        ...
        **  Only support GMT timezone.
            ...
            ref StarDateResult result)
        {
            ...
            if (parseInfo.calendar.CAL_HEBREW)
            {
        ...
        private static Exception GetStarDateParseException(ref StarDateResult result)
        {
            switch (result.failure)
            {
                case ParseFailureKind.ArgumentNull:
                    return new ArgumentNullException(); //return new ArgumentNullException(result.failureArgumentName, Environment.GetResourceString(result.failureMessageID));
                case ParseFailureKind.Format:
                    return new FormatException(); //return new FormatException(Environment.GetResourceString(result.failureMessageID));
                case ParseFailureKind.FormatWithParameter:
                    return new FormatException(); //return new FormatException(Environment.GetResourceString(result.failureMessageID, result.failureMessageFormatArgument));
                case ParseFailureKind.FormatBadStarDateCalendar:
                    return new FormatException(); //return new FormatException(Environment.GetResourceString(result.failureMessageID, result.calendar));
                default:
        ...
        internal static void LexTraceExit(string message, DS dps)
        {
            ...
            if (!_tracingEnabled)
                return;
            BCLDebug.Trace("StarDate", "[StarDate] Lex return {0}, DS.{1}", message, dps);
#endif // _LOGGING
        }
        [Pure]
        [Conditional("_LOGGING")]
        ...
        internal static void PTSTraceExit(DS dps, bool passed)
        {
            ...
            if (!_tracingEnabled)
                return;
            BCLDebug.Trace("StarDate", "[StarDate] ProcessTerminalState {0} @ DS.{1}", passed ? "passed" : "failed", dps);
#endif // _LOGGING
        }
        [Pure]
        [Conditional("_LOGGING")]
        ...
        internal static void TPTraceExit(string message, DS dps)
        {
            ...
            if (!_tracingEnabled)
                return;
            BCLDebug.Trace("StarDate", "[StarDate] TryParse return {0}, DS.{1}", message, dps);
#endif // _LOGGING
        }
        [Pure]
        [Conditional("_LOGGING")]
        ...
        internal static void DTFITrace(StarCulture dtfi)
        {
            ...
            BCLDebug.Trace("StarDate", "      TimeSeparator {0}", Hex(dtfi.TimeSeparator));
            BCLDebug.Trace("StarDate", "      AbbrvDayNames {0}", Hex(dtfi.AbbreviatedDayNames));
            BCLDebug.Trace("StarDate", "   ShortestDayNames {0}", Hex(dtfi.ShortestDayNames));
            BCLDebug.Trace("StarDate", "           DayNames {0}", Hex(dtfi.DayNames));
            BCLDebug.Trace("StarDate", "    AbbrvMonthNames {0}", Hex(dtfi.AbbreviatedMonthNames));
            BCLDebug.Trace("StarDate", "         MonthNames {0}", Hex(dtfi.MonthNames));
            BCLDebug.Trace("StarDate", " AbbrvMonthGenNames {0}", Hex(dtfi.AbbreviatedMonthGenitiveNames));
            BCLDebug.Trace("StarDate", "      MonthGenNames {0}", Hex(dtfi.MonthGenitiveNames));
#endif // _LOGGING
        }
#if _LOGGING
        [Pure]
        ...
        internal static bool _tracingEnabled = BCLDebug.CheckEnabled("StarDate");
#endif // _LOGGING
    }
    ...
    struct __DTString
    {
        ...
        Start:
            if (StarDateParse.IsDigit(m_current))
            {
                // This is a digit.
                ...
                if (Index - start > StarDateParse.MaxStarDateNumberDigits)
                {
        ...
        internal bool GetNextDigit()
        {
            ...
            return (StarDateParse.IsDigit(Value[Index]));
        }
        ...
        {
            Contract.Assert(Index >= 0 && Index < len, "Index >= 0 && Index < len");
            Contract.Assert(StarDateParse.IsDigit(Value[Index]), "IsDigit(Value[Index])");
        ...
        internal DTSubString GetSubString()
        {
            ...
            while (Index + sub.length < len)
            {
                ...
                if (currentType == DTSubStringType.Number)
                {
                    ...
                    if (sub.length > StarDateParse.MaxStarDateNumberDigits)
                    {
    ...
    //
    internal
    struct StarDateToken
    {
        internal StarDateParse.DTT dtt;    // Store the token
    ...
    unsafe struct StarDateRawInfo
    {
        ...
        internal void Init(int* numberBuffer)
        {
            ...
            dayOfWeek = -1;
            era = -1;
            timeMark = StarDateParse.TM.NotSet;
            fraction = -1;
            num = numberBuffer;
        }
        [System.Security.SecuritySafeCritical]  // auto-generated
    ...
    internal enum ParseFailureKind
    {
        ...
        FormatBadStarDateCalendar = 4,  // FormatException when ArgumentOutOfRange is thrown by a Calendar.TryToStarDate().
    };
    ...
    // used to construct a StarDate instance.
    //
    internal
    struct StarDateResult
    {
        internal int Year;
        internal int Month;
        internal int Day;
    ...
    internal struct ParsingInfo
    {
        ...
        internal StarDateParse.MatchNumberDelegate parseNumberDelegate;
    ...
    internal enum TokenType
    {
        ...
        DateWordToken = 10,   // A word that can appear in a StarDate string, but serves no parsing semantics.  E.g. "de" in Spanish culture.
