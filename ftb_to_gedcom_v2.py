#!/usr/bin/env python3
"""
FTB to GEDCOM Converter v2 - Actually works and links families properly
"""
...
class FtbToGedcomV2:
    def __init__(self, ftb_file: str):
        ...
        self.family_connections = {}  # family_id -> {husband_id, wife_id, children: []}
    ...
    def extract_individuals(self):
        """Extract all individuals with their names"""
    ...
    def extract_family_connections(self):
        """Extract family connections using family_individual_connection table"""
        cursor = self.conn.cursor()
        ...
        for conn in connections:
            ...
            # Role types: typically 0=child, 1=husband, 2=wife (may vary)
            if role_type == 1:  # Husband
            ...
            elif role_type == 2:  # Wife  
                family_conn['wife_id'] = individual_id
                if individual_id in self.individuals:
                    self.individuals[individual_id]['families_as_spouse'].append(family_id)
            elif role_type == 0:  # Child
                family_conn['children'].append(individual_id)
                if individual_id in self.individuals:
                    self.individuals[individual_id]['families_as_child'].append(family_id)
            else:
                # Try to determine role by gender if unclear
                if individual_id in self.individuals:
                    gender = self.individuals[individual_id]['gender']
                    if not family_conn['husband_id'] and gender == 'M':
                        family_conn['husband_id'] = individual_id
                        self.individuals[individual_id]['families_as_spouse'].append(family_id)
                    elif not family_conn['wife_id'] and gender == 'F':
                        family_conn['wife_id'] = individual_id
                        self.individuals[individual_id]['families_as_spouse'].append(family_id)
                    else:
                        # Assume child if parents already assigned
                        family_conn['children'].append(individual_id)
                        self.individuals[individual_id]['families_as_child'].append(family_id)
    ...
    def write_gedcom(self, output_file: str):
        """Write GEDCOM file with proper family linkages"""
        with open(output_file, 'w', encoding='utf-8') as f:
            # Write header
            f.write("0 HEAD\n")
            f.write("1 SOUR FTB_to_GEDCOM_v2\n")
            f.write("2 NAME FTB to GEDCOM Converter v2\n")
            ...
            for individual_id, individual in self.individuals.items():
                ...
                # Write family references - THIS IS THE KEY PART THAT WAS MISSING
    ...
    def convert(self, output_file: str):
        ...
        logger.info("Starting FTB to GEDCOM conversion v2")
...
def main():
    ...
    output_file = "new_gedcoms/gaiad_ftb_export_1.ged"
    
    converter = FtbToGedcomV2(ftb_file)
