#!/usr/bin/env python3
"""
MISSING INDIVIDUALS FAMILIES PROCESSOR - FIXED VERSION
...
FIXED: Now properly loads ALL families from GEDCOM, not just the first few.
"""
...
def create_session():
    ...
    session.headers.update({
        'User-Agent': 'Missing Individuals Families Processor FIXED Bot/1.0 (https://github.com/Immanuelle/Gaiad-Genealogy; immanuelle@example.com)'
    })
...
def load_gedcom_data():
    """Load family and individual data from GEDCOM - FIXED VERSION"""
    families = {}
    individuals = {}
    
    print("Loading GEDCOM data - this may take a while...")
...
def process_family_relationships(session, family_id, family_data, mappings, csrf_token):
    ...
    if husband_qid and wife_qid:
        if add_relationship(session, husband_qid, 'P42', wife_qid, csrf_token):  # P42 = Spouse
            ...
            print(f"    Added spouse: {husband_qid} -> {wife_qid}")
        time.sleep(0.1)
        if add_relationship(session, wife_qid, 'P42', husband_qid, csrf_token):
            relationships_added += 1
            print(f"    Added spouse: {wife_qid} -> {husband_qid}")
        time.sleep(0.1)
        ...
        if child_qid:
            ...
            if husband_qid:
                if add_relationship(session, child_qid, 'P47', husband_qid, csrf_token):  # P47 = Father
                    relationships_added += 1
                    print(f"    Added father: {child_qid} -> {husband_qid}")
                time.sleep(0.1)
                if add_relationship(session, husband_qid, 'P20', child_qid, csrf_token):  # P20 = Child
                    relationships_added += 1
                    print(f"    Added child: {husband_qid} -> {child_qid}")
                time.sleep(0.1)
            ...
            if wife_qid:
                if add_relationship(session, child_qid, 'P48', wife_qid, csrf_token):  # P48 = Mother
                    relationships_added += 1
                    print(f"    Added mother: {child_qid} -> {wife_qid}")
                time.sleep(0.1)
                if add_relationship(session, wife_qid, 'P20', child_qid, csrf_token):  # P20 = Child
                    relationships_added += 1
                    print(f"    Added child: {wife_qid} -> {child_qid}")
                time.sleep(0.1)
    
    return relationships_added

def main():
    print("Starting Missing Individuals Families Processor - FIXED VERSION...")
    ...
    # Step 1: Identify newly added individuals (Q152xxx and Q153xxx)
    ...
    # Step 2: Load GEDCOM data - FIXED TO LOAD ALL DATA
    ...
    print(f"\nFixed Missing Individuals Families Processing Complete!")
