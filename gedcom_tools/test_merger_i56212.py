#!/usr/bin/env python3
"""
TEST MERGER FOR @I56212@ DUPLICATE

Merge Q131895 into Q120233 and create redirect
"""
...
def create_session():
    ...
    session.headers.update({
        'User-Agent': 'Test Merger I56212/1.0 (https://github.com/Immanuelle/Gaiad-Genealogy; immanuelle@example.com)'
    })
...
def main():
    print("Test Merger for @I56212@ duplicate")
    print("KEEP: Q120233")
    print("REMOVE: Q131895 (will be redirected)")
    ...
    keep_data = get_entity_data(session, keep_qid)
    redirect_data = get_entity_data(session, redirect_qid)
    
    if keep_data and 'labels' in keep_data and 'en' in keep_data['labels']:
        keep_label = keep_data['labels']['en']['value']
    else:
        keep_label = "No label"
    
    if redirect_data and 'labels' in redirect_data and 'en' in redirect_data['labels']:
        redirect_label = redirect_data['labels']['en']['value']
    else:
        redirect_label = "No label"
    
    print(f"\n{keep_qid}: {keep_label}")
    print(f"{redirect_qid}: {redirect_label}")
