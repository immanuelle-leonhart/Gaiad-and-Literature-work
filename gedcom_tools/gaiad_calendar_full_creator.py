#!/usr/bin/env python3
"""
GAIAD CALENDAR FULL CREATOR

Creates wikibase items for all Gaiad calendar days including intercalary days.
- 13 regular months × 28 days = 364 days
- 7 intercalary days (Horus 1-7) = 7 days
...
def create_session():
    ...
    session.headers.update({
        'User-Agent': 'Gaiad Calendar Full Creator Bot/1.0 (https://github.com/Immanuelle/Gaiad-Genealogy; immanuelle@example.com)'
    })
...
def main():
    print("Starting Gaiad full calendar creation (364 + 7 intercalary days)...")
    ...
    # First create Horus month for intercalary days
    print("Creating Horus month for intercalary days...")
    horus_qid = create_item(session, csrf_token)
    if horus_qid:
        set_label(session, horus_qid, 'en', 'Horus', csrf_token)
        time.sleep(0.2)
        set_label(session, horus_qid, 'ja', 'ホルス', csrf_token)
        time.sleep(0.2)
        set_description(session, horus_qid, 'en', 'Intercalary days in the Gaiad calendar, containing the 7 days of Horus', csrf_token)
        time.sleep(0.2)
        add_statement(session, horus_qid, 'P3', 'Q151959', csrf_token)  # instance of Gaian calendar month
        time.sleep(0.2)
        print(f"  SUCCESS: Created {horus_qid} for Horus")
    else:
        print("  FAILED to create Horus month")
        return
    
    success_count = 0
    error_count = 0
    
    # Create all 364 regular calendar days (13 months × 28 days)
    print("\n=== CREATING REGULAR CALENDAR DAYS ===")
    ...
    # Create the 7 intercalary days (Horus 1-7)
    print("\n=== CREATING INTERCALARY DAYS (HORUS 1-7) ===")
    for horus_day in range(1, 8):  # 1 to 7
        try:
            print(f"  Creating Horus {horus_day}...")
            ...
            # Set English label
            en_label = f"Horus {horus_day}"
            set_label(session, qid, 'en', en_label, csrf_token)
            time.sleep(0.2)
            
            # Set English description
            en_desc = f"{ordinal(horus_day)} intercalary day in the Gaiad calendar"
            set_description(session, qid, 'en', en_desc, csrf_token)
            time.sleep(0.2)
            
            # Set Japanese label
            japanese_label = f"ホルス{horus_day}日"
            set_label(session, qid, 'ja', japanese_label, csrf_token)
            time.sleep(0.2)
            ...
            # Add "part of" relationship to Horus month
            add_statement(session, qid, 'P22', horus_qid, csrf_token)
            time.sleep(0.2)
            
            # Add ordinal in year (365+) - P49
            ordinal_year = 364 + horus_day
            add_numeric_statement(session, qid, 'P49', ordinal_year, csrf_token)
            time.sleep(0.2)
            
            # Add ordinal in month (1-7) - P50
            add_numeric_statement(session, qid, 'P50', horus_day, csrf_token)
            time.sleep(0.2)
            
            # Intercalary days don't map to ISO weeks, but we can set to 53 symbolically
            add_numeric_statement(session, qid, 'P51', 53, csrf_token)
            time.sleep(0.2)
            
            # Set weekday to match position (1-7)
            add_numeric_statement(session, qid, 'P52', horus_day, csrf_token)
            time.sleep(0.2)
            ...
            print(f"    ERROR creating Horus {horus_day}: {e}")
            error_count += 1
    
    print(f"\nGaiad full calendar creation complete!")
    print(f"Successfully created: {success_count} days")
    print(f"Regular days: {success_count - 7}")
    print(f"Intercalary days: 7")
    print(f"Errors: {error_count}")
    print(f"Created Horus month: {horus_qid}")
