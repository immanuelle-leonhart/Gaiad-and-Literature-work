#!/usr/bin/env python3
"""
CHINESE FAMILY RELATIONSHIPS - FINAL VERSION
...
def create_session():
    ...
    session.headers.update({
        'User-Agent': 'Chinese Relationships Final/1.0 (https://github.com/Immanuelle/Gaiad-Genealogy; immanuelle@example.com)'
    })
...
def main():
    print("Starting Chinese relationships with correct property IDs...")
    ...
    print(f"\nLoaded {len(individual_mappings)} individual mappings")
    ...
    for family_id, family_data in families.items():
        ...
        if husband_qid and wife_qid:
            ...
            result = add_relationship_statement(session, husband_qid, wife_qid, 'P42', csrf_token)
            if 'entity' in result:
                success_count += 1
            ...
            result = add_relationship_statement(session, wife_qid, husband_qid, 'P42', csrf_token)
            if 'entity' in result:
                success_count += 1
                print(f"    SUCCESS: Added husband to wife")
            else:
                print(f"    ERROR: Adding husband to wife failed: {result}")
                error_count += 1
            time.sleep(0.5)
        ...
        for child_id in children:
            ...
            if child_qid:
                ...
                # Add father relationship (P47 = father)
                if husband_qid:
                    result = add_relationship_statement(session, child_qid, husband_qid, 'P47', csrf_token)
                    if 'entity' in result:
                        success_count += 1
                        print(f"    SUCCESS: Added father relationship")
                    else:
                        print(f"    ERROR: Adding father failed: {result}")
                        error_count += 1
                    time.sleep(0.5)
                    
                    # Add child to father (P20 = child)
                    result = add_relationship_statement(session, husband_qid, child_qid, 'P20', csrf_token)
                    if 'entity' in result:
                        success_count += 1
                        print(f"    SUCCESS: Added child to father")
                    else:
                        print(f"    ERROR: Adding child to father failed: {result}")
                        error_count += 1
                    time.sleep(0.5)
                
                # Add mother relationship (P48 = mother)
                if wife_qid:
                    result = add_relationship_statement(session, child_qid, wife_qid, 'P48', csrf_token)
                    if 'entity' in result:
                        success_count += 1
                        print(f"    SUCCESS: Added mother relationship")
                    else:
                        print(f"    ERROR: Adding mother failed: {result}")
                        error_count += 1
                    time.sleep(0.5)
                    
                    # Add child to mother (P20 = child)
                    result = add_relationship_statement(session, wife_qid, child_qid, 'P20', csrf_token)
                    if 'entity' in result:
                        success_count += 1
                        print(f"    SUCCESS: Added child to mother")
                    else:
                        print(f"    ERROR: Adding child to mother failed: {result}")
                        error_count += 1
                    time.sleep(0.5)
    
    print(f"\nChinese relationships complete!")
