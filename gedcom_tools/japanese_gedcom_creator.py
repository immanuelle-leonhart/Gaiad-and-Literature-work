...
def create_session():
    ...
    session.headers.update({
        'User-Agent': 'Japanese GEDCOM Creator Bot/1.0 (https://github.com/Immanuelle/Gaiad-Genealogy; immanuelle@example.com)'
    })
...
def create_new_individual(session, data, csrf_token):
    try:
        ...
        # Add other properties using Chinese creator's working configuration
...
def update_mapping_file(gedcom_id, qid):
    try:
        with open('japanese_gedcom_to_qid_mapping.txt', 'a', encoding='utf-8') as f:
            f.write(f"{gedcom_id}\t{qid}\n")
    except Exception as e:
        print(f"Error updating mapping file: {e}")

def main():
    print("Starting Japanese GEDCOM individual creation from @I9998@ onwards...")
    ...
    # Start from I9998 (after highest existing I9997)
    start_id = 9998
    started = False
