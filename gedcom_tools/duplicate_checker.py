...
def check_duplicates():
    ...
    if uuid_duplicates:
        ...
        print(f"✅ UUIDs: All {len(uuids)} are unique")
    ...
    if total_issues == 0:
        ...
        if wiki_duplicates:
            print(f"  ⚠️  Wikidata QID duplicates: {len(wiki_duplicates)}")
        if geni_duplicates:
            print(f"  ⚠️  Geni ID duplicates: {len(geni_duplicates)}")
        if uuid_duplicates:
            print(f"  🚨 UUID duplicates: {len(uuid_duplicates)}")
